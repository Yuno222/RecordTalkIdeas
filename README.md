...
<br/>
また、セキュリティ面では、  
ネットワーク攻撃手法を学習し、対策を施しながらコーディングを行った。  
自動ログイン処理では、cookieにクレデンシャル情報を保存しないことを意識した。
作成したトークンをDBとcookieに保存して、ログイン時に比較することで、安全なログイン処理を実装した。  
<br/>  

## 〇苦労したこと  
当初はWeb周りに関する知識がほぼ皆無だったため、全てにおいて苦労した気がする。
何度も挫折しそうになりながらも、仕事前後の時間を利用して学習を進め、なんとか1つのアプリを完成させることができた。

コーディングする前の段階で、
機能やサイト構成、デザイン、DB設計などを、1から考えるのが大変だった。  

また、初期はいいね機能を実装する予定は無かったため、  
webアプリ完成後に後付けで実装することになった。  
しかし、調べるうちにjavascriptやjQueryの知識が必要だと分かり、苦労した。 
