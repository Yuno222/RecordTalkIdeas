...
ナンパを趣味にしている知り合いが、iPhoneのメモ帳に会話の内容を殴り書きしているのを目撃して、  
...
## 工夫した点  
###①機能面
どのような人がこのサービスを使うのかを意識して、機能やレイアウトを実装しました。  
いいね機能ではAjaxを利用して、ユーザ側にストレスを感じさせないようにしました。

- ログインがめんどくさい　→ 自動ログイン機能  
- 過去の会話の記録を残したいのか、なんとなく閃いた会話のアイディアを残したいのか　→ ジャンル分け  
- 後から見返したい　→ ソート機能  
- 実際に使ってみた手応えを残したい→　いいね機能  
...
自動ログインの部分では、作成したトークンをDBとcookieに保存して比較することで、安全なログイン処理を実装しました。


課題  
いいね順でソート  
検索機能  
会話内容作成画面でも非同期機能を実装する  
